...
const diente = db.diente;
const Op = db.Sequelize.Op;

// crear y guardar un nuevo registro
    exports.create = (req, res) => {
        // Validar solicitud
        if (!req.body.descripciondiente) {
          res.status(400).send({
            message: res
          });
          return;
        ...
        // Crear ,
        const dientes = {
            descripciondiente: req.body.descripciondiente
        };
      
        // Guardar diente en la base de datos el registro
        diente .create(dientes)
          .then(data => {
            res.send(data);
          })
          .catch(err => {
     ...
     exports.muestratodo = (req, res) => {
        ...
        diente.findAll({where: condition})
          .then(data => {
            res.send(data);
          })
          .catch(err => {
